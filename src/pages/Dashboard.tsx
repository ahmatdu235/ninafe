...
import { FileText, Upload, ShieldCheck, GraduationCap, CreditCard, Eye, Loader2, Camera } from "lucide-react";
...
// --- INTERFACE CORRIGÉE POUR ACCEPTER LES PROPS DE APP.TSX ---
...
export default function Dashboard(props: DashboardProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 dark:text-slate-100 font-sans text-slate-900 transition-colors">
      ...
      {/* FIX : On passe bien les props isDark et setIsDark au Header */}
      <DashboardHeader 
          type="candidat" 
          isDark={props.isDark} 
          ...
          <TabsContent value="profile" className="mt-6 space-y-6">
            <Card className="border-brand-blue/20 shadow-sm dark:bg-slate-900 dark:border-slate-800">
                ...
                <CardContent className="grid gap-6 md:grid-cols-3">
                    <DocumentBox title="CV" icon={<FileText />} isMandatory={true} fileUrl={docs.cv} onUpload={(e: any) => handleFileUpload(e, 'cv')} />
                    <DocumentBox title="CNI / Passeport" icon={<CreditCard />} isMandatory={true} fileUrl={docs.idCard} onUpload={(e: any) => handleFileUpload(e, 'idCard')} />
                    <DocumentBox title="Diplôme" icon={<GraduationCap />} isMandatory={false} fileUrl={docs.diploma} onUpload={(e: any) => handleFileUpload(e, 'diploma')} />
                </CardContent>
            </Card>
              ...
              <CardContent>
                <div className="space-y-4">
                    {myApplications.map(app => (
                        <div key={app.id} className="flex items-center justify-between p-4 border rounded-lg bg-white dark:bg-slate-950 dark:border-slate-700">
                            <div>
        ...
        <div className={`border rounded-lg p-4 flex flex-col justify-between transition-all ${fileUrl ? 'bg-blue-50 border-blue-200 dark:bg-blue-950/30 dark:border-blue-900' : 'bg-white border-slate-200 border-dashed dark:bg-slate-950/50 dark:border-slate-700'}`}>
            <div>
                <div className="flex justify-between items-start mb-3">
                    ...
                    {isMandatory ? <Badge variant="destructive" className="text-xs">Requis</Badge> : <Badge variant="outline" className="text-xs">Optionnel</Badge>}
                </div>
